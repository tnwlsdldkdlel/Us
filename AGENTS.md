...
## 에이전트 작업 프로세스
- **명확화 우선**: 사용자 지시가 불명확하면 작업 착수 전에 반드시 추가 설명을 요청합니다.
- **불확실성 지도**: 모든 응답 말미에 자신 없는 부분, 단순화 가능성, 추가 정보가 필요한 지점을 정리합니다.
- **작업 로깅**: 각 요청마다 `__prompts/_task/`에 `YYYY-MM-DD-HH-MM_간단한-작업-제목.md` 형식의 로그 파일을 작성합니다.
- **작업 계획 문서화**: 각 요청에 대해 `__prompts/_plan/`에 Todo/Plan/Working/Result를 포함한 `YYYY-MM-DD-HH-MM_작업명.md` 파일을 업데이트합니다.
- **파일 동기화**: 결과 보고 전에 관련 파일을 다시 읽어 최신 상태를 확인합니다.
- **히스토리 추적**: 필요 시 `__prompts` 내부 문서로 과거 요청 맥락을 확인합니다.
- **PRD 관리**: 제품 요구사항 정의서는 `__prompts/_docs/prd.md`에 저장합니다.
